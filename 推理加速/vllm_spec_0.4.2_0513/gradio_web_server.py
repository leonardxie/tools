...
logger = build_logger("gradio_web_server", f'run_logs/gradio/gradio_web_server_{datetime.datetime.now().date().strftime("%Y_%m_%d")}.log')
...
def get_conv_log_filename():
    ...
    name = os.path.join(LOGDIR, "run_logs", "gradio", f"{t.year}-{t.month:02d}-{t.day:02d}-conv.json")
    return name
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=22004)
    parser.add_argument(
        "--share",
        action="store_true",
