...
logger = build_logger("model_worker", f'run_logs/api_server/openai_api_server_{datetime.datetime.now().date().strftime("%Y_%m_%d")}.log')
...
class AppSettings(BaseSettings):
    ...
    controller_address: str = "http://localhost:22001"
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    if request.whether_use_speculate:
        if request.ngram_txt and len(request.ngram_txt) < 6:
            request.ngram_txt = "参考文档：" + request.ngram_txt
    ...
    gen_params = await get_gen_params(
        ...
        echo=False,
        stop=request.stop,
        whether_use_speculate=request.whether_use_speculate,
        ngram_txt=request.ngram_txt,
    )
    ...
    logging.info(f"max_tokens of vllm：{max_new_tokens}")
...
async def chat_completion_stream_generator(
    ...
    for i in range(n):
        ...
        async for content in generate_completion_stream(gen_params, worker_addr):
            ...
            if len(delta_text) == 0:
                delta_text = None
            # 应对最后finish reason为stop，但是delta_text不为空的情况,进行拆分
            if content.get("finish_reason", None) == "stop" and delta_text is not None:
                choice_data = ChatCompletionResponseStreamChoice(
                    index=i,
                    delta=DeltaMessage(content=delta_text),
                    finish_reason=None,
                )
                choice_data_finish = ChatCompletionResponseStreamChoice(
                    index=i,
                ...
                choice_data_finish = None
            chunk = ChatCompletionStreamResponse(
                id=id, choices=[choice_data], model=model_name
            )

            if choice_data_finish:
                chunk_finish = ChatCompletionStreamResponse(
                    id=id, choices=[choice_data_finish], model=model_name
                )
                finish_stream_events.append(chunk_finish)
                # continue
            if delta_text is None:
                if content.get("finish_reason", None) is not None:
                    finish_stream_events.append(chunk)
                continue
            # print(chunk.model_dump_json(exclude_unset=True, exclude_none=True))
            yield f"data: {chunk.model_dump_json(exclude_unset=True, exclude_none=True)}\n\n"
    # There is not "content" field in the last delta message, so exclude_none to exclude field "content".
    for finish_chunk in finish_stream_events:
        # print(finish_chunk.model_dump_json(exclude_unset=True, exclude_none=True))
        yield f"data: {finish_chunk.model_dump_json(exclude_unset=True, exclude_none=True)}\n\n"
    yield "data: [DONE]\n\n"
...
def create_openai_api_server():
    ...
    parser.add_argument("--port", type=int, default=22003, help="port number")
    parser.add_argument(
        "--controller-address", type=str, default="http://localhost:22001"
    )
    parser.add_argument(
