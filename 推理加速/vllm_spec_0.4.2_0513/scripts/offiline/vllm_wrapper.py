...
class vLLMWrapper:
    def __init__(self,
        ...
        # parser = AsyncEngineArgs.add_cli_args(kwargs.get('parser', None))
        # parser.set_defaults(model=model_dir)
        # parser.set_defaults(draft_model=draft_model_dir)
        # parser.set_defaults(tokenizer=model_dir)
        # parser.set_defaults(tensor_parallel_size=tensor_parallel_size)
        # parser.set_defaults(speculate_length=5)
        # parser.set_defaults(gpu_memory_utilization=gpu_memory_utilization)
        # parser.set_defaults(trust_remote_code=trust_remote_code)
        # parser.set_defaults(dtype=dtype)
        # # parser.set_defaults(disable_log_stats=True)
        # parser.set_defaults(quantization=quantization)
        #
        ...
        self.model = LLM(
            model=model_dir,
            draft_model=draft_model_dir if len(draft_model_dir) > 0 else None,
            tokenizer=model_dir,
    ...
    def chat(self,
        ...
        **kwargs):
        ...
        sampling_kwargs = {
            ...
            "max_tokens": kwargs.get('max_tokens', generation_config.max_new_tokens),
            "repetition_penalty": kwargs.get('repetition_penalty', generation_config.repetition_penalty),
            "logits_processors": kwargs.get('logits_processors', None),
            "whether_use_speculate": kwargs.get('whether_use_speculate', True),
            "ngram_txt": kwargs.get("ngram_txt", None),
        }
